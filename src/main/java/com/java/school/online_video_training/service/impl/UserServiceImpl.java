...
public class UserServiceImpl implements UserService {
	 ...
	 * @Override
	 * 
	 * @Transactional public User applyForAuthor(UserRegistrationDTO
	 * registrationDTO) { log.info("Processing author application for user: {}",
	 * registrationDTO.getUsername());
	 * 
	 * // Find existing user User user =
	 * userRepository.findByUsername(registrationDTO.getUsername()) .orElseThrow(()
	 * -> new ResourceNotFoundException("User not found"));
	 * 
	 * // Check if user is already an author // if (user.isAuthor()) { // throw new
	 * ApiException(HttpStatus.BAD_REQUEST, "User is already an author"); // } if
	 * (user.getIsAuthor()) { throw new ApiException(HttpStatus.BAD_REQUEST,
	 ...
	 * 
	 * // Set application status user.setAuthorApprovalRequested(true);
	 * user.setAuthorApprovalStatus("PENDING"); //user.setAuthor(false);
	 * user.setIsAuthor(false); user.setAuthorApproved(false);
	 * 
	 * // Generate tokens for admin approval String approveToken =
	 * jwtUtils.generateJwtToken(user.getEmail() + "_APPROVE"); String rejectToken =
	 * jwtUtils.generateJwtToken(user.getEmail() + "_REJECT");
	 * 
	 * // Store tokens user.setApproveToken(approveToken);
	 * user.setRejectToken(rejectToken);
	 * 
	 * // Send admin notification email try { String adminEmail =
	 * "Boysoy331@gmail.com"; String subject =
	 * "ALERT: New Author Application Requires Your Attention";
	 * 
	 ...
	 * <p>Dear Admin,</p>
	 * 
	 * <p>A user has applied to become an author. Please review the details
	 * below:</p>
	 * 
	 * <div
	 * style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin: 20px 0;"
	 * > <p style="margin: 5px 0;"><strong>Username:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Email:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Gender:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Phone:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Education:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Address:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Current Role:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Bio:</strong> %s</p> <p
	 * style="margin: 5px 0;"><strong>Expertise:</strong> %s</p> </div>
	 * 
	 * <div style="text-align: center; margin: 30px 0;"> <a href="%s" style="
	 * background-color: #4CAF50; color: white; padding: 12px 25px; text-decoration:
	 * none; border-radius: 5px; margin-right: 10px; font-weight: bold; display:
	 * inline-block; ">APPROVE</a>
	 * 
	 * <a href="%s" style=" background-color: #f44336; color: white; padding: 12px
	 * 25px; text-decoration: none; border-radius: 5px; font-weight: bold; display:
	 * inline-block; ">REJECT</a> </div>
	 * 
	 * <p style="color: #666; font-size: 12px; text-align: center;"> Note: This is
	 * an automated message. Please do not reply. </p>
	 * 
	 * <p style="text-align: center;"> Best regards,<br> Your Application Team </p>
	 * </body> </html> """, user.getUsername(), user.getEmail(),
	 * registrationDTO.getGender(), registrationDTO.getPhoneNumber(),
	 * registrationDTO.getEducation(), registrationDTO.getAddress(),
	 ...
	 * "https://84de-154-214-2-4.ngrok-free.app /author/reject?token=" +
	 * rejectToken); // Update email service to send HTML content
	 * emailService.sendVerificationEmail(adminEmail, subject, text, true);
	 * log.info("Admin notification email sent for author application: {}",
	 * user.getEmail()); } catch (Exception e) {
	 * log.error("Failed to send admin notification email: {}", e.getMessage());
	 * throw new RuntimeException("Failed to send admin notification email", e); }
	 * 
	 * // Save only the application status and tokens return
...
//		}
		...
		try {
			...
			String text = String.format("""
					...
					user.getRoles().stream().map(Role::getName).collect(Collectors.joining(", ")),
					registrationDTO.getAuthorBio(), registrationDTO.getAuthorExpertise(), approveLink, rejectLink);
	...
	public String handleAuthorApproval(String token) {
		User user = userRepository.findByApproveToken(token).or(() -> userRepository.findByRejectToken(token))
	 ...
	 * @Override
	 * 
	 * @Transactional public String handleAuthorApproval(String token) { if
	 * (!jwtUtils.validateJwtToken(token)) { throw new
	 * ApiException(HttpStatus.BAD_REQUEST, "Invalid or expired token"); }
	 * 
	 * String email = jwtUtils.getUserNameFromJwtToken(token); User user =
	 * userRepository.findByEmail(email) .orElseThrow(() -> new
	 * ApiException(HttpStatus.BAD_REQUEST, "User not found"));
	 * 
	 * if (token.endsWith("_APPROVE")) { // Admin approved - update user with
	 * temporary data user.setGender(user.getTempGender());
	 * user.setPhoneNumber(user.getTempPhoneNumber());
	 * user.setEducation(user.getTempEducation());
	 * user.setAddress(user.getTempAddress()); user.setBio(user.getTempAuthorBio());
	 * user.setExpertise(user.getTempExpertise());
	 * 
	 * // Clear temporary fields user.setTempGender(null);
	 * user.setTempPhoneNumber(null); user.setTempEducation(null);
	 * user.setTempAddress(null); user.setTempAuthorBio(null);
	 * user.setTempExpertise(null);
	 * 
	 * // Update status user.setAuthorApprovalStatus("APPROVED");
	 * //user.setAuthor(true); user.setAuthorApprovalRequested(true);
	 * user.setAuthorApproved(true);
	 * 
	 * // Add AUTHOR role Role authorRole = roleRepository.findByName("AUTHOR")
	 * .orElseThrow(() -> new ApiException(HttpStatus.BAD_REQUEST,
	 * "Role Author not found")); user.getRoles().add(authorRole);
	 * 
	 * userRepository.save(user); return "Author application approved successfully";
	 * } else if (token.endsWith("_REJECT")) { // Admin rejected - clear all
	 * temporary data user.setTempGender(null); user.setTempPhoneNumber(null);
	 * user.setTempEducation(null); user.setTempAddress(null);
	 * user.setTempAuthorBio(null); user.setTempExpertise(null);
	 * 
	 * // Update status user.setAuthorApprovalRequested(false);
	 * user.setAuthorApprovalStatus("REJECTED"); // user.setAuthor(false);
	 * user.setIsAuthor(false); user.setAuthorApproved(false);
	 * 
	 * userRepository.save(user); return "Author application rejected successfully";
	 * }
	 * 
	...
	/*
	 * @Override public Page<String> getPhoto(Map<String, String> photos) {
	 * UserFilter imageFilter = new UserFilter();
	 * 
	 * if (photos.containsKey("photo")) { String name = photos.get("photo");
	 * imageFilter.setPhoto(name); }
	 * 
	 * int pageLimit = PageUtil.DEFAULT_PAGE_LIMIT; if
	 * (photos.containsKey(PageUtil.PAGE_LIMIT)) { pageLimit =
	 * Integer.parseInt(photos.get(PageUtil.PAGE_LIMIT)); }
	 * 
	 * int pageNumber = PageUtil.DEFAULT_PAGE_NUMBER; if
	 * (photos.containsKey(PageUtil.PAGE_NUMBER)) { pageNumber =
	 * Integer.parseInt(photos.get(PageUtil.PAGE_NUMBER)); }
	 * 
	 * UserSpec spec = new UserSpec(imageFilter);
	 * 
	 * Pageable pageable = PageUtil.getPageable(pageNumber, pageLimit);
	 * 
	 * Page<User> page = userRepository.findAll(spec, pageable); Page<String>
	 * imagePaths = page.map(video -> video.getPhoto());
	 * 
	 * return imagePaths; }
	 */
