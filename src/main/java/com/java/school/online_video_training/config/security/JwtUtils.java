...
public class JwtUtils {
	...
	public String generateJwtToken(String username, List<String> authorities) {
	    return Jwts.builder()
	        ...
	        .claim("authorities", authorities.stream()
	            .map(role -> Map.of("authority", role))
	...
	public String generateToken(User user, String action) {
		// Set the subject as the user's email and include the action (approve or
		// reject)
		return Jwts.builder().setSubject(user.getEmail()) // Subject is the user's email
				...
				.signWith(key(), SignatureAlgorithm.HS256).compact(); // Create the token
	}
